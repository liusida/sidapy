...
__host__ unsigned int getReqThreadUnitsCountCCL(unsigned int rows, unsigned int cols) { return ceil_h_d((float)rows / 2) * ceil_h_d((float)cols / 2); }
...
__device__ unsigned int getRealCol(unsigned int cols, unsigned int effectiveblockIndex) { return threadIdx.x + (effectiveblockIndex % (cols - 2)); }

__global__ void processMapSampleHillDale(unsigned int *d_map, unsigned int rows, unsigned int cols, char *d_hillDaleRemaining,
                                         unsigned int threadBlockBatchIndex, unsigned int *d_aux_map) {
...
__global__ void calculateAverage(unsigned int *d_map, unsigned int mapSize, unsigned int *d_average, unsigned int lastThreadBlockLastIndex,
                                 unsigned int lastThreadBlockIndex, char odd, unsigned int threadBlockBatchIndex) {
...
__global__ void assignBlockNumbers(unsigned int *d_map, unsigned int rows, unsigned int cols, unsigned int blockMatrixRowLength,
...
__global__ void processMapSampleCCLZeroIndexHorizontal0(unsigned int *d_map, char *d_ifLesserFound, unsigned int rows, unsigned int cols,
...
__global__ void processMapSampleCCLOneIndexHorizontal1(unsigned int *d_map, char *d_ifLesserFound, unsigned int rows, unsigned int cols,
                                                       unsigned int blockMatrixRowLength, unsigned int threadBlockBatchIndex) {
        unsigned int effectiveBlockId = blockIdx.x + MAX_NO_OF_THREADBLOCKS * threadBlockBatchIndex;
...
__global__ void processMapSampleCCLZeroIndexVertical2(unsigned int *d_map, char *d_ifLesserFound, unsigned int rows, unsigned int cols,
                                                      unsigned int blockMatrixRowLength, unsigned int threadBlockBatchIndex) {
        unsigned int effectiveBlockId = blockIdx.x + MAX_NO_OF_THREADBLOCKS * threadBlockBatchIndex;
        unsigned int n = effectiveBlockId + 1, nFloor = n;
        ...
        unsigned int rootLinearIndex =
            (((n - 1) / blockMatrixRowLength) * (cols)) + (cols + (2 * n) - 2 - ((nFloor / blockMatrixRowLength) * ((cols % 2) > 0)));
...
__global__ void processMapSampleCCLOneIndexVertical3(unsigned int *d_map, char *d_ifLesserFound, unsigned int rows, unsigned int cols,
                                                     unsigned int blockMatrixRowLength, unsigned int threadBlockBatchIndex) {
        unsigned int effectiveBlockId = blockIdx.x + MAX_NO_OF_THREADBLOCKS * threadBlockBatchIndex;
        unsigned int n = effectiveBlockId + 1, nFloor = n;
        ...
        unsigned int rootLinearIndex =
            (((n - 1) / blockMatrixRowLength) * (cols)) + (cols + (2 * n) - 1 - ((nFloor / blockMatrixRowLength) * ((cols % 2) > 0)));
        ...
        while (true) {
                ...
                if (threadBlockBatchCountFloat <= 1) {
                        ...
                        while ((h_hillDaleRemaining) && (iterations < NUM_ITERATIONS)) {
                                ...
                                processMapSampleHillDale<<<extraThreadBlockCount, dimMap>>>(d_map, rows, cols, d_hillDaleRemaining, threadBlockBatchCount,
                                                                                            d_aux_map);
                ...
                odd = ~(lastThreadBlockElementCount &&
                        (!(lastThreadBlockElementCount & (lastThreadBlockElementCount - 1)))); // To check if lastThreadBlockElementCount is a power
                                                                                               // of 2

                threadBlockBatchCountFloat = (float)reqThreadUnits2 / MAX_NO_OF_THREADBLOCKS;
                if ((threadBlockBatchCountFloat <= 1)) {
                        calculateAverage<<<reqThreadUnits2, THREADS_PER_BLOCK>>>(d_map, currMapSize, d_average1, lastTBLastIndex, (reqThreadUnits2 - 1), odd,
                                                                                 0);
                } else {
                        threadBlockBatchCount = (unsigned int)threadBlockBatchCountFloat;
                        extraThreadBlockCount = reqThreadUnits1 - threadBlockBatchCount * MAX_NO_OF_THREADBLOCKS;
                        for (i = 0; i < threadBlockBatchCount; i++) {
                                calculateAverage<<<MAX_NO_OF_THREADBLOCKS, THREADS_PER_BLOCK>>>(d_map, currMapSize, d_average1, lastTBLastIndex,
                                                                                                (reqThreadUnits2 - 1), odd, i);
                        }
                        calculateAverage<<<extraThreadBlockCount, THREADS_PER_BLOCK>>>(d_map, currMapSize, d_average1, lastTBLastIndex, (reqThreadUnits2 - 1),
                                                                                       odd, threadBlockBatchCount);
                }
                ...
                if (reqThreadUnits2 < SERIAL_SUM_LIMIT) {
                        ...
                        calculateAverage<<<reqThreadUnits3, THREADS_PER_BLOCK>>>(d_average1, reqThreadUnits2, d_average2, lastTBLastIndex,
                ...
                while (h_ifLesserFound[0] | h_ifLesserFound[1] | h_ifLesserFound[2] | h_ifLesserFound[3]) {
                        ...
                        threadBlockBatchCountFloat = (float)reqThreadUnits2 / MAX_NO_OF_THREADBLOCKS;
                        if (threadBlockBatchCountFloat <= 1) {
                                processMapSampleCCLOneIndexHorizontal1<<<reqThreadUnits2, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                               ceil_h_d((float)(cols - 1) / 2), 0);
                        } else {
                                threadBlockBatchCount = (unsigned int)threadBlockBatchCountFloat;
                                extraThreadBlockCount = reqThreadUnits2 - threadBlockBatchCount * MAX_NO_OF_THREADBLOCKS;
                                for (i = 0; i < threadBlockBatchCount; i++) {
                                        processMapSampleCCLOneIndexHorizontal1<<<MAX_NO_OF_THREADBLOCKS, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                              ceil_h_d((float)(cols - 1) / 2), i);
                                }
                                processMapSampleCCLOneIndexHorizontal1<<<extraThreadBlockCount, 4>>>(d_map, d_ifLesserFound, rows, cols,
                        ...
                        if (threadBlockBatchCountFloat <= 1) {
                                ...
                                extraThreadBlockCount = reqThreadUnits3 - threadBlockBatchCount * MAX_NO_OF_THREADBLOCKS;
                                for (i = 0; i < threadBlockBatchCount; i++) {
                                        processMapSampleCCLZeroIndexVertical2<<<MAX_NO_OF_THREADBLOCKS, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                             ceil_h_d((float)cols / 2), i);
                                }
                                processMapSampleCCLZeroIndexVertical2<<<extraThreadBlockCount, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                    ceil_h_d((float)cols / 2), threadBlockBatchCount);
                        }

                        threadBlockBatchCountFloat = (float)reqThreadUnits4 / MAX_NO_OF_THREADBLOCKS;
                        if (threadBlockBatchCountFloat <= 1) {
                                processMapSampleCCLOneIndexVertical3<<<reqThreadUnits4, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                             ceil_h_d((float)(cols - 1) / 2), 0);
                        } else {
                                threadBlockBatchCount = (unsigned int)threadBlockBatchCountFloat;
                                extraThreadBlockCount = reqThreadUnits4 - threadBlockBatchCount * MAX_NO_OF_THREADBLOCKS;
                                for (i = 0; i < threadBlockBatchCount; i++) {
                                        processMapSampleCCLOneIndexVertical3<<<MAX_NO_OF_THREADBLOCKS, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                            ceil_h_d((float)(cols - 1) / 2), i);
                                }
                                processMapSampleCCLOneIndexVertical3<<<extraThreadBlockCount, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                   ceil_h_d((float)(cols - 1) / 2), threadBlockBatchCount);
                        }

                        threadBlockBatchCountFloat = (float)reqThreadUnits1 / MAX_NO_OF_THREADBLOCKS;
                        if (threadBlockBatchCountFloat <= 1) {
                                processMapSampleCCLZeroIndexHorizontal0<<<reqThreadUnits1, 4>>>(d_map, d_ifLesserFound, rows, cols, ceil_h_d((float)cols / 2),
                                                                                                0);
                        } else {
                                threadBlockBatchCount = (unsigned int)threadBlockBatchCountFloat;
                                extraThreadBlockCount = reqThreadUnits1 - threadBlockBatchCount * MAX_NO_OF_THREADBLOCKS;
                                for (i = 0; i < threadBlockBatchCount; i++) {
                                        processMapSampleCCLZeroIndexHorizontal0<<<MAX_NO_OF_THREADBLOCKS, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                               ceil_h_d((float)cols / 2), i);
                                }
                                processMapSampleCCLZeroIndexHorizontal0<<<extraThreadBlockCount, 4>>>(d_map, d_ifLesserFound, rows, cols,
                                                                                                      ceil_h_d((float)cols / 2), threadBlockBatchCount);
                        }
