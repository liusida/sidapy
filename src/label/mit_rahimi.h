// modified from http://alumni.media.mit.edu/~rahimi/connected/
// add a threashold
// change new_label type to size_t
#ifndef _CONNECTED_H
#define _CONNECTED_H
...
class ConnectedComponents {
      public:
        ConnectedComponents(int soft_maxlabels) : labels(soft_maxlabels) { clear(); }
        void clear() {
                std::fill(labels.begin(), labels.end(), Similarity());
                highest_label = 1; // Sida: leave 0 for nothing
        }
        template <class Tin, class Tlabel, class Comparator, class Boolean>
        int connected(const Tin *img, Tlabel *out, int width, int height, Comparator, Boolean K8_connectivity, Tin threashold);
      ...
      private:
        ...
        bool is_root_label(int id) { return (labels[id].sameas == id); }
        int root_of(int id) {
        ...
        bool is_equivalent(int id, int as) { return (root_of(id) == root_of(as)); }
        ...
        template <class Tin, class Tlabel, class Comparator, class Boolean>
        void label_image(const Tin *img, Tlabel *out, int width, int height, Comparator, Boolean K8_connectivity, Tin threashold);
        template <class Tlabel> int relabel_image(Tlabel *out, int width, int height);
...
template <class Tin, class Tlabel, class Comparator, class Boolean>
int ConnectedComponents::connected(const Tin *img, Tlabel *labelimg, int width, int height, Comparator SAME, Boolean K8_connectivity, Tin threashold) {
        label_image(img, labelimg, width, height, SAME, K8_connectivity, threashold);
        return relabel_image(labelimg, width, height);
}

template <class Tin, class Tlabel, class Comparator, class Boolean>
void ConnectedComponents::label_image(const Tin *img, Tlabel *labelimg, int width, int height, Comparator SAME, const Boolean K8_CONNECTIVITY, Tin threashold) {
        const Tin *row = img;
        ...
        label(&row[0]) = (row[0] < threashold ? 0 : new_label());
        ...
        for (int c = 1, r = 0; c < width; ++c) {
                if (SAME(row[c], row[c - 1]))
                        ...
                        label(&row[c]) = (row[c] < threashold ? 0 : new_label());
        }
...
template <class Tlabel> int ConnectedComponents::relabel_image(Tlabel *labelimg, int width, int height) {
        int newtag = 0;
